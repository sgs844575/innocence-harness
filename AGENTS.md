...
19. **Shell layout.** Gray sidebar column (265px, full height) plus a dark main area with 12px left rounding floating over the page background. The sidebar top row owns the logo chip, the back/forward history arrows (disabled stubs), and the sidebar-collapse toggle — never put them in the title bar. The 48px title bar inside the main area reads, in order: conversation title (15px bold) → project/route/branch pills (28px tall, sunken, 14px gaps) → "…" menu → right cluster (editor chip, terminal/panel toggles, separator, custom window controls). The companion activity capsule is 319px (`CAPSULE_WIDTH`) with the reference git-panel rhythm (18px padding, 26px rows, 11px row gaps); the chat column stays `clamp(48rem, 72vw, 62rem)`. The font identity is monospace: `--font-sans` and `--font-mono` share the Maple Mono / Cascadia Code stack (CJK falls back to YaHei); body text is 14px.
