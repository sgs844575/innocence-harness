...
19. **Shell layout.** Gray sidebar column (265px, full height) plus a dark main area with 12px rounded corners floating over the page background. The sidebar top row owns the logo chip, the back/forward history arrows (disabled stubs), and the sidebar-collapse toggle — never put them in the title bar. The 48px title bar inside the main area reads, in order: a left 265px segment (logo, history arrows, and the new-session shortcut that only appears when the sidebar is collapsed) → conversation title (14px bold) → project/route/branch pills (28px tall, sunken, 14px gaps) → right cluster (editor chip, terminal/panel toggles, separator, custom window controls). The left segment shrinks to 142px and its background fades from sidebar-tone to transparent (revealing the main panel beneath) as the sidebar collapses; the sidebar column itself animates width 265↔0 and disappears entirely (no rail strip) — all of this is on the reduced-motion branch. The companion activity capsule is 319px (`CAPSULE_WIDTH`) with the reference git-panel rhythm (18px padding, 26px rows, 11px row gaps); the chat column stretches to the full width (no clamp) with the capsule subtracted in the frame layout. The chat timeline exposes a clickable dash ladder in the left gutter (vertical-center, horizontally-center) that jumps to the corresponding scroll position. The font identity is monospace: `--font-sans` and `--font-mono` share the Maple Mono / Cascadia Code stack (CJK falls back to YaHei); body text is 14px.
