...
22. **Popups (shadcn radius).** All popover / dropdown / menu / dialog / drawer / floating card surfaces use the shadcn `rounded-(--radius-pop)` token (current value 8px per <https://ui.shadcn.com/>); do not hardcode `rounded-[10px]` etc. Body/chat surfaces stay `rounded-[12px]` (`--radius-card`). Anchored cards (CodeBlock, ReviewPanel, RoutePanel, ConflictView, ConflictTrio, AutomationView) keep their larger surface radii as cards, not popups.
