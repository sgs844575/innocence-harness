// Memory plugin (batch 4B task 1): a dual-root durable-note store with the
// write/list/read tool trio. Factory form (same staged shape as the creation
...
/** Creates the memory plugin for one session: registers the three tools bound
 *  to the option getters (roots resolve per call, so a workspace switch
 *  between sessions is honored without rebuilding the plugin). */
