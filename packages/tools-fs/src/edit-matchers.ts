// Edit 匹配器：行尾归一 + 策略链宽容匹配（策略与语义移植自同作者的参考
// 项目，重写适配本仓）。匹配全部发生在 LF 归一域；每个策略产出候选，唯
// 一候选才命中，多候选判歧义；replace_all 跳过宽匹配器（防误替换）。
// 写回侧由 edit.ts 按原文件行尾风格还原。
...
/** 宽匹配器：replace_all 下跳过（唯一性失守的替换风险过高）。 */
...
/** 逐级尝试：精确 → 七级宽容策略；先命中先停，唯一候选才 matched。 */
export function findEditMatch(input: {
  content: string;
  search: string;
  replaceAll: boolean;
}): EditMatchResult {
  const exact = collectSubstringCandidates(input.content, input.search);
  if (exact.length > 0) {
...
/** 匹配域归一：CRLF 折 LF（孤 \r 保留）。 */
export function normalizeLineEndings(content: string): string {
  return content.replaceAll("\r\n", "\n");
}

/** 转义归一策略下，new_string 需同样反转义后再写入。 */
...
/** 弯引号策略下，new_string 跟随文件实际的直/弯引号风格。 */
...
/** 剥掉模型从 Read 输出带回来的「行号: / 行号\t」前缀；任一行不带前缀则整串不适用。 */
...
/** 按原文 LF/CRLF 出现占比判定写回行尾风格（CRLF 占多即 CRLF）。 */
