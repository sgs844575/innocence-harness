...
vi.mock("./sessions", () => ({
  ...
  archiveSession: mocks.archiveSession, pinSession: mocks.pinSession, reorderSessions: mocks.reorderSessions, moveSession: mocks.moveSession, reorderSidebarContainers: mocks.reorderSidebarContainers,
...
describe("sidebar mutation IPC durability", () => {
  ...
  it("routes pin toggles through the sidebar store and broadcasts the new state", () => {
    const handler = mocks.handles.get(IPC.sidebarPin);
    handler?.({}, "s1", true);
    expect(mocks.pinSession).toHaveBeenCalledWith("s1", true);
    expect(mocks.broadcastSidebar).toHaveBeenCalledOnce();
  });

  it("propagates a failed pin mutation without publishing sidebar state", () => {
    mocks.pinSession.mockImplementation(() => { throw new Error("sidebar state was not saved"); });
    const handler = mocks.handles.get(IPC.sidebarPin);
