    ...
    <RadixPopover.Root open={open} onOpenChange={setOpen}>
      ...
      <RadixPopover.Portal>
        <RadixPopover.Content
          ...
          align="end"
          sideOffset={6}
          className={`dropdown-in z-50 min-w-40 max-w-[var(--radix-popover-content-available-width)] rounded-(--radius-pop) border border-(--color-border) bg-(--color-popup) p-1 shadow-(--shadow-pop) ${fullWidth ? "w-[var(--radix-popover-trigger-width)]" : ""}`}
        >
          {options.map((option) => (
            <button
              key={option.value}
              ...
              className="flex w-full items-center gap-2 rounded px-2.5 py-1.5 text-left text-(--color-foreground) hover:bg-(--color-hover) focus-visible:bg-(--color-hover) focus-visible:outline-none"
            >
