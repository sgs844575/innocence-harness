        ...
        <DialogContent aria-label="全局搜索" className="fixed left-1/2 top-[18vh] z-50 w-[min(620px,calc(100vw-32px))] -translate-x-1/2 overflow-hidden rounded-2xl border border-(--color-app-border) bg-(--color-app-panel) shadow-(--shadow-pop)">
          ...
          <div className="flex items-center gap-2 border-b border-(--color-app-hairline) px-4 py-3">
            ...
            <input
              ...
              className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-(--color-app-muted)"
            />
            <kbd className="rounded border border-(--color-app-border) px-1.5 py-0.5 text-[10px] text-(--color-app-muted)">Esc</kbd>
          </div>
          <div className="max-h-[min(65vh,480px)] overflow-y-auto p-2">
            <SearchGroup icon={<Command size={13} />} label="操作" empty={matchingActions.length === 0}>
              {matchingActions.map((action) => (
                <button key={action.id} type="button" className="flex w-full items-center rounded-lg px-3 py-2 text-left text-[12px] hover:bg-(--color-app-bubble)" onClick={() => { action.onSelect(); onOpenChange(false); }}>
                  {action.label}
                </button>
              ))}
            </SearchGroup>
            <SearchGroup icon={<ListTodo size={13} />} label="任务" empty={matchingSessions.length === 0}>
              {matchingSessions.map((session) => (
                <button key={session.id} type="button" className="flex w-full items-center rounded-lg px-3 py-2 text-left text-[12px] hover:bg-(--color-app-bubble)" onClick={() => { onSelectSession(session.id); onOpenChange(false); }}>
                  {session.title}
                </button>
              ))}
            </SearchGroup>
            <SearchGroup icon={<FileCode2 size={13} />} label="文件" empty={matchingFiles.length === 0}>
              {matchingFiles.map((file) => (
                <button key={file} type="button" disabled={!onSelectFile} className="flex w-full items-center rounded-lg px-3 py-2 text-left font-mono text-[11px] hover:bg-(--color-app-bubble) disabled:cursor-not-allowed disabled:opacity-50" onClick={() => { onSelectFile?.(file); onOpenChange(false); }}>
                  {file}
                </button>
              ))}
            </SearchGroup>
            {matchingActions.length === 0 && matchingSessions.length === 0 && matchingFiles.length === 0 && (
              <p className="px-3 py-6 text-center text-xs text-(--color-app-muted)">没有匹配结果</p>
            )}
          </div>
        </DialogContent>
      </DialogPortal>
...
function SearchGroup({ icon, label, empty, children }: { icon: React.ReactNode; label: string; empty: boolean; children: React.ReactNode }): React.JSX.Element | null {
  ...
  return (
    <section className="mb-2 last:mb-0">
      <h2 className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-(--color-app-muted)">{icon}{label}</h2>
      {children}
    </section>
  );
}
